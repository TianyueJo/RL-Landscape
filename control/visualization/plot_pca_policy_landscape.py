#!/usr/bin/env python3
"""
Use PCA to reduce policy parameters to 2D and visualize in 3D space:
- x, y: PCA-reduced 2D coordinates
- z: episode reward
"""
...
def flatten_params(state_dict: dict):
    """
    Flatten all trainable parameters in state_dict into a large vector.
    Returns:
    ...
    if not flats:
        raise RuntimeError("state_dict is empty? No tensor parameters")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize policy parameter space and episode reward using PCA"
    )
    parser.add_argument(
        "--models-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-path",
        type=Path,
        default=Path("landscape_plots/pca_policy_landscape_3d.png"),
        help="Path to save PNG image",
    )
    parser.add_argument(
        "--output-html",
        type=Path,
        default=Path("landscape_plots/pca_policy_landscape_3d.html"),
...
def get_episode_reward_from_features(
    ...
    # Use the file with maximum step
...
def collect_all_policies(
    ...
    """Collect all policy parameters and metadata"""
    all_params = []
    all_metadata = []
    ...
    for task_id in task_ids:
        ...
        if params is not None:
            ...
            # Try to get from feature files, if not, get from best_info.json
    ...
    if retrain_models_dir is not None and retrain_models_dir.exists():
        ...
        for task_id, jr_index, step_size in sorted(retrain_tasks):
            ...
            if params is not None:
                ...
                # If model file doesn't exist, try to get reward from feature files
                episode_reward = get_episode_reward_from_features(
                    data_dir, task_id, jr_index, step_size
                )
                if episode_reward is not None:
                    # If no model file but feature file exists, use original policy parameters as placeholder
...
def main() -> None:
    ...
    print(f"\nReducing dimensions to {args.pca_components}D using PCA...")
    pca = PCA(n_components=args.pca_components, random_state=42)
    params_reduced = pca.fit_transform(params_matrix)
    print(f"Reduced dimension shape: {params_reduced.shape}")
    ...
    if original_indices:
        orig_x = params_reduced[original_indices, 0]
        orig_y = params_reduced[original_indices, 1]
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(
                size=8,
                color='#000000',  # Black for original policies
                opacity=0.8,
    ...
    for step_size in sorted(args.step_sizes):
        ...
        retrain_x = params_reduced[step_indices, 0]
        retrain_y = params_reduced[step_indices, 1]
    ...
    fig.update_layout(
        title=dict(
            text=f'Policy Parameter Space PCA Visualization (3D)<br>' +
    ...
    ax.set_title(
        f"Policy Parameter Space PCA Visualization (3D)\n"
